...
function addInterest(interest) {
  Interests.define({ name: interest });
}
...
function addProfile({ firstName, lastName, bio, title, interests, projects, picture, email, role }) {
  ...
  // Create the profile.
  Profiles.define({ firstName, lastName, bio, title, picture, email });
  // Add interests and projects.
  interests.map(interest => ProfilesInterests.define({ profile: email, interest }));
  projects.map(project => ProfilesProjects.define({ profile: email, project }));
...
function addProject({ name, homepage, description, interests, picture }) {
  ...
  Projects.define({ name, homepage, description, picture });
  interests.map(interest => ProjectsInterests.define({ project: name, interest }));
  // Make sure interests are defined in the Interests collection if they weren't already.
  interests.map(interest => addInterest(interest));
}
